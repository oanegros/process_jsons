...
for path in processed:
    ...
    # df = pd.melt(df, id_vars=['time'], value_vars=["10loglongevity", 'fission_rate', "fusion_rate", 'rep', 'HOST_V_PER_OXPHOS', 'host_division_volume'])
    # print(df)
    g = sns.pairplot(data=df, vars=["10loglongevity", 'fission_rate', "fusion_rate", 'rep', 'HOST_V_PER_OXPHOS', 'host_division_volume'], kind='kde', hue='time', palette='viridis')
    # g = sns.FacetGrid(df, col="variable", hue="time", sharex=False,sharey=False, palette='viridis')
...
fig, ax = plt.subplots()
g = sns.pairplot(data=df, vars=["10loglongevity", 'fission_rate', "fusion_rate", 'rep', 'HOST_V_PER_OXPHOS', 'host_division_volume'], kind='hist', hue='path')
# df = df[(df['time of birth'] > 0)]
